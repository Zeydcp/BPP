        ...
        IEnumerator secondCorountine(List<(int, int)> swapList)
        {
            ...
            foreach (string address in reformat)
            {
                ...
                for (int i = 0; i < difference; i++)
                {
                    ...
                    index -= new Vector3(0, 0.4f, 0);
                    index = new(index.x, (float) Math.Round(index.y, 1), index.z);

                    dataset.AddressObjects[address] = index;
            ...
            index = dataset.AddressObjects[reformat[0]] + new Vector3((reformat.Count-1)/2f, 0.4f, 0);

            index = new((float) Math.Round(index.x, 1), (float) Math.Round(index.y, 1), index.z);

            selection = Instantiate(inputBlock, index, Quaternion.identity, parent).transform;
            selection.localScale = new(reformat.Count, 1, 1);
        ...
        foreach((int, int) swap in swapList)
        {
            ...
            foreach (GameObject obj in important2)
            {
                ...
                if (obj.transform.position.y <= dataset.AddressObjects[address].y)
                {
                    ...
                    dataset.AddressObjects[address] = new(index.x, (float) Math.Round(index.y, 1), index.z);
            ...
            for (int i = 0; i < difference; i++)
            {
                ...
                adr1pos = new(adr1pos.x, (float) Math.Round(adr1pos.y, 1), adr1pos.z);
                yield return new WaitForSeconds(1/5f);
            }
            
            dataset.AddressObjects[address1] = adr1pos;
