...
public class TextAdder : MonoBehaviour
{
    ...
    public void ReadStringInput(string input)
    {
        ...
        if (input == "swap")
            textBox.transform.parent.parent.tag = input;
        
        textBox.text = dataset.Checker(positions, input);
        Resize(textBox.transform.parent.parent);
    }
    ...
    public void Turnoncanvas(Transform selection)
    {
        ...
        {
            Vector3 position = textBox.transform.parent.parent.position;
    ...
    private List<Vector3> Calculatepositions()
    {
        ...
        int xscale = (int) Math.Round(textBox.transform.parent.parent.localScale.x);
        Vector3 pos = textBox.transform.parent.parent.position;
        float xpos = pos.x;
        xpos -= (xscale - 1) / 2f;
    ...
    private void Resize(Transform block)
    {
        if (block.localScale.x % 1 != 0.9f) 
        {
            ...
            Transform canvas = block.GetChild(0).GetChild(0);
